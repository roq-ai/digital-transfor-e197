...
function DomainListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<DomainInterface[]>(
    ...
    () =>
      getDomains({
        relations: ['dimension', 'question.count'],
      }),
  );
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  {hasAccess('dimension', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>dimension</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('dimension', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/dimensions/view/${record.dimension?.id}`}>
